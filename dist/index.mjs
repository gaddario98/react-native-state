...
export { atomStateGenerator } from '@gaddario98/react-state';
    ...
    getItem: async (key) => {
        try {
            if (Platform.OS === "web") {
                ...
                if (compressedValue) {
                    const decompressed = lzStringExports.decompress(compressedValue);
                    return decompressed !== null && decompressed !== void 0 ? decompressed : null;
                }
                return null;
            }
            else {
    ...
    setItem: async (key, value) => {
        try {
            if (!value)
                return;
            if (Platform.OS === "web") {
                const compressedValue = lzStringExports.compress(value);
...
const setReactNativeStorage = () => setCustomStorage(storage);
